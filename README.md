...
Visual Studio Code で使う場合は `LaTeX Workshop` の利用をおすすめしますが、`LaTeX Workshop` の自動ビルドは使用しないことを強く推奨します。`LaTeX Workshop` を使用せずにターミナルから直接 `latexmk` を呼び出すこともできますし、そちらのほうが意図しないエラーが生じた際の強制停止がやりやすいためです。本リポジトリはコンパイル方法の特性上、時々エラーが生じますが、LaTeX Workshop の自動ビルドを使用している場合、強制停止にやや煩雑な操作が必要となります。

一方で、`LaTeX Workshop` はLaTeX文書のPDFのプレビューをVS Codeで表示する機能を備えており、このビューアーは現時点で日本語表示に関する問題をほとんど起こさない数少ないビューアーでもあります。以上のことから、このリポジトリには [settings.json](.vscode/settings.json) を追加し、その中で自動ビルドをオフにしています。
